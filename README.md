...
- 👀 **4가지 보기 방식** — 컬렉션별 / 모델별 / **작품·캐릭터별** / 전체 사진 스트림(무한 스크롤)
- 🔍 **통합 검색** (`/` 단축키) — 행사·작품·캐릭터·모델을 한 번에. 공식명이 영어여도
  **한국어 통칭**으로 찾힙니다 (`Comic World` → 서코·부코·수코, `PlayX4` → 플엑·플레이엑스포)
...
- 🛠️ **웹 기반 관리자** (`/admin`, 비밀번호 보호)
  ...
  - **이중 백업** — D1 메타데이터 JSON + 별도 R2 버킷의 사진 원본 배치 스냅샷·복원 (최근 3개 자동 유지)
...
> 기존 `moments-in-light` 운영 DB는 과거 런타임 부트스트랩으로 스키마가 먼저 생성되어
> 마이그레이션 기록과 실제 스키마가 어긋날 수 있습니다. 최초 전환 시에는 원격 스키마를
> 확인하고 기존 마이그레이션을 기준선으로 등록한 뒤 `migrations apply`를 실행해야 합니다.
> 확인 없이 `0001`부터 재적용하면 중복 컬럼 오류가 발생할 수 있습니다.
...
>
> 📝 **무엇이 바뀌었는지 보고 싶다면** → [**CHANGELOG.md**](CHANGELOG.md)
